...
class WebpackCLI {
    ...
    async loadConfig(options) {
        ...
        const loadConfigByPath = async (configPath, argv = { env: {} }) => {
            ...
            try {
                ...
                try {
                    ...
                    options = (await eval(`import("${(0, node_url_1.pathToFileURL)(configPath)}")`)).default;
                }
                catch (err) {
        ...
        if (options.config && options.config.length > 0) {
            const loadedConfigs = await Promise.all(options.config.map((configPath) => loadConfigByPath(node_path_1.default.resolve(configPath), options.argv)));
        ...
        const resolveExtends = async (config, configPaths, extendsPaths) => {
            ...
            const loadedConfigs = await Promise.all(extendsPaths.map((extendsPath) => loadConfigByPath(node_path_1.default.resolve(extendsPath), options.argv)));
